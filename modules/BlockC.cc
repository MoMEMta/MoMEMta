...
class BlockC: public Module {
    public:
        ...
        virtual Status work() override {
            ...
            const double gamma1 = -(2*E2*pTx - 2*X*pTx - *s12*cosphi3*sinthe3 + *s123*cosphi3*sinthe3)/denom;
            ...
            const double b01 = -0.5   - (beta1*gamma4  + beta4*gamma1)*sinthe3*cosphi3  - (beta2*gamma4  + beta4*gamma2)*sinthe3*sinphi3  - (beta3*gamma4  + beta4*gamma3)*costhe3;
            ...
            for (unsigned int i = 0; i < e1.size(); i++) {
                ...
                double jacobian = E3*sinthe3/(16* SQ(M_PI) * SQ(sqrt_s) * std::abs(chi*(E2*p1z - E1*p2z) + 2*E2*(A + B + C + D))); 
